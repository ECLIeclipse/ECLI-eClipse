...
#if defined(NDEBUG)
# error "eclipseCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbbaeeb5618243b1646c0234113b0fc59ed1e6590e029ce1db457de8160b1f44b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eclipseCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eclipseCoin Signed Message:\n";
...
{
    // eclipseCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 5000 blocks
    nSubsidy >>= (nHeight / 5000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 200 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eclipseCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eclipseCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbbaeeb5618243b1646c0234113b0fc59ed1e6590e029ce1db457de8160b1f44b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1445619849, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "EclipseCoin 2015";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f20e2e92a913ab8e4dad8b1cfdca9e741924501fb023a3b069c0cb25bf57dc5efa67fc946da8313a061826d6b84c71c8eb8f1a6ded655594ea50ee8fe917fc40") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1445619849;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1051528;
        ...
        assert(block.hashMerkleRoot == uint256("0xa4e571de489b87ad1936fe33e5732a12268a449451cee193cb99348630965ed7"));
...
unsigned char pchMessageStart[4] = { 0xdb, 0x8b, 0xa8, 0x3a }; // eclipseCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eclipseCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eclipseCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eclipseCoinMiner(CWallet *pwallet)
{
    printf("eclipseCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eclipsecoin-miner");
    ...
    try { loop {
        ...
        printf("Running eclipseCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("eclipseCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eclipseCoinMiner, pwallet));
}
